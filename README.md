...
For all emulators, you will have to setup your controller(s). You can do this by exiting fullscreen on GuyStation, opening a terminal on the Ubuntu Desktop, becoming root (`sudo -s`), and typing the command to run emulator you want to open (without a game). The correct command to start each emulator can be found in `guystation/systems/<SYSTEM>/metadata.json` as the entry for `command` where `<SYSTEM>` is the system you want to open the emulator for (e.g. gba, nds, ngc). For example, `guystation/systems/wii/metadata.json` contains the line `"command": "dolphin-emu",`, so I would run `dolphin-emu` to open the Dolphin emulator and configure my controllers. The one exception is Nintendo 64. In that case, you will want to run the command `m64py` to edit the configuration.
